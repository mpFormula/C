...
#if defined (Use_Mpfr) || defined (Use_Mpfi) || defined (Use_Mpdec) || defined (Use_Mpq) || defined (Use_Arb)
    x=SourceString;
#endif
    (*DestMatrix)(0, 0) = x ;
}
...
#ifdef Use_Arb
extern void EigenLib_arf_SetPrecision(int prec)
{
    mpfr_set_default_prec(prec);
}
#endif // Use_Arb
...
{
	switch (what) {
		case mp_const_block: (*x).block(i-1, j-1, p, q) = (*result) ; break;
...
{
	switch (what) {
		case mp_const_block: (*result) = (*x).block(i-1, j-1, p, q); break;
	...
	switch (what) {
		case mp_const_GT:  // >
			switch (CmpMode) {
				...
				case 3: *result = (long) ((*w).array()  >   (*x)(0,0)).count(); break;
			} break;
		case mp_const_LT:  // <
			switch (CmpMode) {
				case mp_const_All: *result = ((*w).array()  <   (*x)(0,0)).all(); break;
