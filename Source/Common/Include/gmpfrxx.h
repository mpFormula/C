...
/*
TO BE ADDED (after MPFR 2.3, up to 3.1.2):
...
Functions 1 arg, 2 results:
//mpfr_modf in MPFR 2.4.
//mpfr_sin_cos in MPFR 2.3.
...
struct __gmp_rec_sqrt_function // not in gmpfrxx
{
  static void eval(mpfr_ptr f, mpfr_srcptr g)
  { mpfr_rec_sqrt(f, g, MpFrC::get_rnd()); }
};

struct __gmp_li2_function // not in gmpfrxx
{
  static void eval(mpfr_ptr f, mpfr_srcptr g)
  { mpfr_li2(f, g, MpFrC::get_rnd()); }
};


struct __gmp_ai_function // not in gmpfrxx
{
  static void eval(mpfr_ptr f, mpfr_srcptr g)
  { mpfr_ai(f, g, MpFrC::get_rnd()); }
};


struct __gmp_digamma_function // not in gmpfrxx
{
  static void eval(mpfr_ptr f, mpfr_srcptr g)
  { mpfr_digamma(f, g, MpFrC::get_rnd()); }
};


struct __gmp_fmod_function // not in gmpfrxx
{
  static void eval(mpfr_ptr f, mpfr_srcptr g, mpfr_srcptr h)
  { mpfr_fmod(f, g, h, MpFrC::get_rnd()); }
};


struct __gmp_modf_function // not in gmpfrxx
{
  static void eval(mpfr_ptr f, mpfr_ptr g, mpfr_srcptr h)
  { mpfr_modf(f, g, h, MpFrC::get_rnd()); }
};


struct __gmp_sin_cos_function // not in gmpfrxx
{
  static void eval(mpfr_ptr f, mpfr_ptr g, mpfr_srcptr h)
  { mpfr_sin_cos(f, g, h, MpFrC::get_rnd()); }
};


struct __gmp_sinh_cosh_function // not in gmpfrxx
{
  static void eval(mpfr_ptr f, mpfr_ptr g, mpfr_srcptr h)
  { mpfr_sinh_cosh(f, g, h, MpFrC::get_rnd()); }
};
...
struct __gmp_rand_function
{
  ...
  //***************** DH begin ***************************
  static void eval(mpfr_ptr f, mpfr_ptr f2, gmp_randstate_t s, unsigned long int prec)
  {
    // note code change from gmpxx.h
    unsigned long int precf = mpfr_get_prec(f);
    if (prec == precf) {
      ...
      mpfr_set_prec(f, prec);
      mpfr_grandom(f, f2, s, MpFrC::get_rnd());
      mpfr_prec_round(f, precf, MpFrC::get_rnd());
      mpfr_prec_round(f2, precf, MpFrC::get_rnd());
    }
  }
  //***************** DH end ***************************
...
public:
  ...
  //***********************DH begin ************************
  bool nan_p() const  { return mpfr_nan_p(mp); }  // not in gmpfrxx
  bool inf_p() const  { return mpfr_inf_p(mp); }  // not in gmpfrxx
  bool number_p() const  { return mpfr_number_p(mp); }  // not in gmpfrxx
  bool zero_p() const  { return mpfr_zero_p(mp); }  // not in gmpfrxx
  bool regular_p() const  { return mpfr_regular_p(mp); }  // not in gmpfrxx

  bool buildopt_decimal_p() const  { return mpfr_buildopt_decimal_p(); }  // not in gmpfrxx
  bool buildopt_tls_p() const  { return mpfr_buildopt_tls_p(); }  // not in gmpfrxx
  bool buildopt_gmpinternals_p() const  { return mpfr_buildopt_gmpinternals_p(); }  // not in gmpfrxx
...
//  mpz_set_mpfr(z, f.get_mpfr_t());
  mpfr_get_z(z, f.get_mpfr_t(), MpFrC::get_rnd());
}
...
inline void __gmp_set_expr(mpz_ptr z, const __gmp_expr<mpfr_t, T> &expr)
{
  ...
  mpfr_get_z(z, temp.get_mpfr_t(), MpFrC::get_rnd());
...
__GMP_DEFINE_UNARY_FUNCTION(rec_sqrt, __gmp_rec_sqrt_function) // not in gmpfrxx.h
__GMP_DEFINE_UNARY_FUNCTION(li2, __gmp_li2_function) // not in gmpfrxx.h
__GMP_DEFINE_UNARY_FUNCTION(ai, __gmp_ai_function) // not in gmpfrxx.h
__GMP_DEFINE_UNARY_FUNCTION(digamma, __gmp_digamma_function) // not in gmpfrxx.h

__GMP_DEFINE_BINARY_FUNCTION(fmod, __gmp_fmod_function) // not in gmpfrxx.h
__GMP_DEFINE_BINARY_FUNCTION(modf, __gmp_modf_function) // not in gmpfrxx.h
__GMP_DEFINE_BINARY_FUNCTION(sin_cos, __gmp_sin_cos_function) // not in gmpfrxx.h
__GMP_DEFINE_BINARY_FUNCTION(sinh_cosh, __gmp_sinh_cosh_function) // not in gmpfrxx.h
