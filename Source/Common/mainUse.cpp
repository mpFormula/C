...
{
    int n = 14;
    EigenLib_Arb_SetPrecision(40);
    mpNumMatrixPtr mPtr=0, xPtr=0, zPtr=0;

    EigenLib_Arb_Init(&mPtr);
    EigenLib_Arb_Init(&xPtr);
    EigenLib_Arb_Init(&zPtr);


    SetSpecialValue_Arb(n, 1, mp_FillLinear, xPtr);
    EigenLib_Arb_Print("Here is the matrix x:", xPtr);
    boost_Arb_ElementaryFunctions1(mp_const_Tgamma, zPtr, xPtr);
    EigenLib_Arb_Print("Here is the matrix z:", zPtr);


//    SetSpecialValue_Arb(n, n, mp_setRandom_nm, mPtr);
//    EigenLib_Arb_Print("Here is the matrix m:", mPtr);
//    EigenLib_Arb_Inverse(xPtr, mPtr, 0);
//    EigenLib_Arb_Print("Here is the matrix x:", xPtr);
//    BasicArithmetic_Arb(mp_const_MatrixProduct, mPtr, xPtr, zPtr);
//    EigenLib_Arb_Print("Here is the product m * x:", zPtr);

    EigenLib_Arb_Clear(&mPtr);
    EigenLib_Arb_Clear(&xPtr);
    EigenLib_Arb_Clear(&zPtr);
}

void cplx_T1()
{
    int n = 4;
    EigenLib_Arb_SetPrecision(30);
    mpNumMatrixPtr mPtr=0, xPtr=0, zPtr=0;

    cplx_EigenLib_Arb_Init(&mPtr);
    cplx_EigenLib_Arb_Init(&xPtr);
    cplx_EigenLib_Arb_Init(&zPtr);

    cplx_SetSpecialValue_Arb(n, n, mp_setRandom_nm, mPtr);
    cplx_EigenLib_Arb_Print("Here is the matrix m:", mPtr);
    cplx_EigenLib_Arb_Inverse(xPtr, mPtr, 0);
    cplx_EigenLib_Arb_Print("Here is the matrix x:", xPtr);
    cplx_BasicArithmetic_Arb(mp_const_MatrixProduct, mPtr, xPtr, zPtr);
    cplx_EigenLib_Arb_Print("Here is the product m * x:", zPtr);

    cplx_EigenLib_Arb_Clear(&mPtr);
    cplx_EigenLib_Arb_Clear(&xPtr);
    cplx_EigenLib_Arb_Clear(&zPtr);
}
